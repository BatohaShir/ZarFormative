...
export const SearchInput = React.memo(function SearchInput({ className }: SearchInputProps) {
    ...
    <div ref={containerRef} className={`relative ${className || ""}`}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-background border rounded-lg shadow-lg z-50 max-h-80 overflow-y-auto">
          {isLoading && (
            <div className="p-4 text-center text-muted-foreground text-sm">Хайж байна...</div>
          )}
          ...
          {!isLoading &&
            ...
            results.map((listing) => {
              ...
              const priceDisplay = formatListingPrice(
                listing.price,
                listing.currency,
          ...
          {showNoResults && (
            <div className="p-4 text-center text-muted-foreground text-sm">Хайлт олдсонгүй</div>
          )}
        </div>
      )}
    </div>
